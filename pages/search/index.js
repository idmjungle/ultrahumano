...
export default function SearchPage({ posts }) {
  ...
  return (
    <Layout title={`Resultados de ${searchName}`}>
      <div id="column_info" className="col-sm-4 left_small vh-100 overflow-auto column_morph">
          <div
            className="ultra_light full_rounded py-2 px-3 mt-3 top_left"
          >
            Busqueda
          </div>
          <div
            className="ultra_light full_rounded py-2 px-3 mt-3 top_left"
          >
            {(posts.length > 1) ? posts.length + " Resultados" : 'Solo un Resultado'} con {searchName}
          </div>
      </div>
